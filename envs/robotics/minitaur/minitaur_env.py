...
from chaosbreaker.envs.robotics.minitaur import minitaur, minitaur_derpy, motor
from chaosbreaker.envs.robotics.minitaur import minitaur_rainbow_dash
...
class MinitaurGymEnv(gym.Env):
    ...
    def action_space(self):
        ...
        return FloatBox(-action_high, action_high)
    ...
    def observation_space(self):
        ...
        observation_low = (- upper_bound - OBSERVATION_EPS)
        return FloatBox(observation_low, observation_high)
    ...
    def render(self, mode="rgb_array", height=RENDER_HEIGHT, width=RENDER_WIDTH):
        if mode != "rgb_array":
            return np.array([])
        base_pos = self.minitaur.GetBasePosition()
        ...
        rgb_array = rgb_array[:, :, :3].reshape(3, height, width)
        return rgb_array
    ...
    def _termination(self):
        ...
        return self.is_fallen() or distance > self._distance_limit or self._env_step_counter >= self.max_num_steps
    ...
    def _reward(self):
        ...
        # print(np.abs(self.minitaur.GetMotorAcceleration()).sum() * 0.05)
...
class MinitaurPixEnv(MinitaurGymEnv):
    def __init__(self, render_height=64, render_width=64, **kwargs):
        self.render_height = render_height
        self.render_width = render_width
        super(MinitaurPixEnv, self).__init__(**kwargs)

    @property
    def observation_space(self):
        return IntBox(low=0, high=255, shape=(3, self.render_height, self.render_width), dtype="uint8")

    def step(self, action):
        obs, rew, done, info = super(MinitaurPixEnv, self).step(action)
        obs = self.render(height=self.render_height, width=self.render_width)
        return obs, rew, done, info

    def reset(self):
        super(MinitaurPixEnv, self).reset()
        return self.render(height=self.render_height, width=self.render_width)
