...
from neuroblast import datasets
from chaosbreaker.envs import calculateScore
...
def load_conditional(type='low'):
    if type == 'low':
        ...
        path = os.path.join(os.path.dirname(cwd), 'envs', 'opt.test.logP-SA')
        import csv
        ...
        cwd = os.path.dirname(__file__)
        path = os.path.join(os.path.dirname(cwd), 'envs', 'zinc_plogp_sorted.csv')
        import csv
        with open(path, 'r') as fp:
            reader = csv.reader(fp, delimiter=',', quotechar='"')
...
def load_scaffold():
    cwd = os.path.dirname(__file__)
    path = os.path.join(os.path.dirname(cwd), 'envs', 'mols_vocab.txt')  # gdb 13
    with open(path, 'r') as fp:
        reader = csv.reader(fp, delimiter=',', quotechar='"')
...
class Mols(gym.Env):
    def __init__(self,
        ...
        if self.dataset is None:
            raise ValueError("dataset \"{}\" not found in [{}]".format(dataset_name, ", ".join(all_datasets.keys())))
    ...
    def step(self, action):
        ...
        if self.check_valency():
            if self.mol.GetNumAtoms() + self.mol.GetNumBonds() - mol_old.GetNumAtoms() - mol_old.GetNumBonds() > 0:
        ...
        if terminate_condition or self.force_final:
            ...
            if self.is_conditional:
                info['reward_valid'] = self.conditional[-1]  # FIXME temp change
            else:
    ...
    def _add_bond(self, action):
        ...
        ----------
        action
        [first_node, second_node, bong_type_id]
    ...
    def get_observation(self):
        ...
        # self.mol, very important!! no aromatic
...
# TODO(Bowen): num rings is a discrete variable, so what is the best way to calculate the reward?
