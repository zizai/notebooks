...
def dreamer_loss(policy, model, dist_class, train_batch):
    ...
    policy.stats_dict = compute_dreamer_loss(
        train_batch["obs"],
        train_batch["actions"],
        train_batch["rewards"] * policy.config["reward_scale"],
        policy.model,
...
def action_sampler_fn(policy, model, input_dict, state, explore, timestep):
    ...
    # Custom Exploration
    if timestep <= policy.config["learning_starts"]:
        logp = [0.0]
        ...
        eps = max(0, policy.config["explore_noise"] - policy.global_timestep / 10000)
        noise = eps * torch.randn(*action.shape, device=action.device)
        action += noise
...
DreamerTorchPolicy = build_torch_policy(
    ...
    make_model_and_action_dist=build_dreamer_model,
