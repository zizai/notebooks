...
from policy.actor_critic import mlp
...
class DreamerModel(TorchModelV2, nn.Module):
    def __init__(self, obs_space, action_space, num_outputs, model_config,
        ...
        if isinstance(obs_space, Discrete) or len(obs_space.shape) <= 2:
            obs_dim = obs_space.shape[0]
            self.encoder = mlp((obs_dim,) + (self.hidden_size,) * num_hidden_layers + (self.hidden_size,), nn.ReLU, nn.Tanh)
            self.decoder = DenseDecoder(self.stoch_size + self.deter_size, obs_dim, num_hidden_layers, self.hidden_size, act=nn.Tanh)
            self.reward = DenseDecoder(self.stoch_size + self.deter_size, 1, num_hidden_layers, self.hidden_size)
            self.dynamics = RSSM(
                self.action_size,
                self.hidden_size,
        ...
        self.actor = ActionDecoder(self.stoch_size + self.deter_size,
                                   self.action_size, 2, self.hidden_size)
        self.value = DenseDecoder(self.stoch_size + self.deter_size, 1, num_hidden_layers,
                                  self.hidden_size)
        self.state = None
