...
################################################################################
from utils.loss_landscape import h5_util
...
def set_weights(net, weights, directions=None, step=None):
    ...
    if directions is None:
        ...
        for (p, w, d) in zip(net.parameters(), weights, changes):
            p.data = w + torch.as_tensor(d).type(type(w))
...
def setup_direction(dir_file, dir_type, net, same_dir, xignore, xnorm, y, yignore, ynorm):
    """
    ...
    if exists(dir_file):
        ...
        if (y and 'ydirection' in f.keys()) or 'xdirection' in f.keys():
            f.close()
            print ("direction file is already set up: %s" % dir_file)
            return
        f.close()
    else:
        ...
        if y:
            if same_dir:
                ...
                ydirection = create_random_direction(net, dir_type, yignore, ynorm)
...
def name_direction_file(dir_type, idx, model_file, same_dir, xignore, xnorm, y, yignore, ynorm):
    dir_file = model_file

    dir_file += '_' + dir_type
    if xignore:
        dir_file += '_xignore=' + xignore
    if xnorm:
        dir_file += '_xnorm=' + xnorm

    # name for ydirection
    if y:
        if yignore:
            dir_file += '_yignore=' + yignore
        if ynorm:
            dir_file += '_ynorm=' + ynorm
        if same_dir:  # ydirection is the same as xdirection
            dir_file += '_same_dir'

    # index number
    if idx > 0:
        dir_file += '_idx=' + str(idx)
