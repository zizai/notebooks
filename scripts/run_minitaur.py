...
import numpy as np
import chaosbreaker.envs.robotics.minitaur.minitaur_env as minitaur_gym_env
...
def SinePolicyExample(log_path=None):
    ...
    for step_counter in range(steps):
        ...
        if done:
            print(obs)
            print(sum_reward)
            print(environment.minitaur.GetBasePosition())
            print(environment._env_step_counter)
            environment.reset()
