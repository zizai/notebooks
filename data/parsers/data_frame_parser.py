        ...
        if isinstance(pp, MolPreprocessor):
            ...
            for row in tqdm(df.itertuples(index=False), total=df.shape[0]):
                ...
                try:
                    ...
                    mol_features = pp.get_input_features(mol)
                ...
                if features is None:
                    if isinstance(mol_features, tuple):
                        num_features = len(mol_features)
                    else:
                        num_features = 1
                ...
                if isinstance(mol_features, tuple):
                    for i in range(len(mol_features)):
                        features[i].append(mol_features[i])
                else:
                    features[0].append(mol_features)
                if self.labels is not None:
